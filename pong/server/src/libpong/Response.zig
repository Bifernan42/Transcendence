...
pub fn attachStream(self: *Response, stream: Stream) void {
    self.stream = stream;
}

pub fn appendSlice(self: *Response, items: []const u8) !void {
...
pub fn jsonSerialize(self: *Response, object: anytype) ![]const u8 {
...
fn internalLength(self: *Response) usize {
    return self.buffer.items.len;
}

fn getRemaining(self: *Response) []const u8 {
    const buff = self.internalSlice();
    const len = self.internalLength();
    if (self.last_res) |response| {
        if (response.len == len) return buff[0..0];
        return buff[response.len..];
    }
    return buff[0..self.internalLength()];
}

pub fn clearLastRetainRemaining(self: *Response) void {
